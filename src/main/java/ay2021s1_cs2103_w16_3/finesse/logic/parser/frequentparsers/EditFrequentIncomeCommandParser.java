...
/**
 * Parses input arguments and creates a new EditFrequentIncomeCommand object
 */
public class EditFrequentIncomeCommandParser implements Parser<EditFrequentIncomeCommand> {
    /**
     * Parses the given {@code String} of arguments in the context of the EditFrequentIncomeCommand
     * and returns an EditFrequentIncomeCommand object for execution.
     * @throws ParseException if the user input does not conform the expected format
     */
    public EditFrequentIncomeCommand parse(String args) throws ParseException {
        requireNonNull(args);
        ...
        if (argMultimap.getValue(PREFIX_AMOUNT).isPresent()) {
            editFrequentIncomeDescriptor.setAmount(ParserUtil.parseAmount(argMultimap.getValue(PREFIX_AMOUNT).get()));
        }

        parseCategoriesForEdit(argMultimap.getAllValues(PREFIX_CATEGORY))
                .ifPresent(editFrequentIncomeDescriptor::setCategories);

        if (!editFrequentIncomeDescriptor.isAnyFieldEdited()) {
            throw new ParseException(EditFrequentIncomeCommand.MESSAGE_NOT_EDITED);
        }

        return new EditFrequentIncomeCommand(index, editFrequentIncomeDescriptor);
    }
