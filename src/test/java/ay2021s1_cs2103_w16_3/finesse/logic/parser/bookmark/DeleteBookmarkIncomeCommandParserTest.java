 ...
 * outside of the DeleteBookmarkIncomeCommand code. For example, inputs "1" and "1 abc" take the
 * same path through the DeleteBookmarkIncomeCommand, and therefore we test only one of them.
 * The path variation for those two cases occur inside the ParserUtil, and
 * therefore should be covered by the ParserUtilTest.
 */
public class DeleteBookmarkIncomeCommandParserTest {
    private DeleteBookmarkIncomeCommandParser parser = new DeleteBookmarkIncomeCommandParser();

    @Test
    public void parse_validArgs_returnsDeleteCommand() {
        assertParseSuccess(parser, "1", new DeleteBookmarkIncomeCommand(INDEX_FIRST));
    }

    @Test
    public void parse_invalidArgs_throwsParseException() {
        assertParseFailure(parser, "a", String.format(MESSAGE_INVALID_COMMAND_FORMAT,
                DeleteBookmarkIncomeCommand.MESSAGE_USAGE));
    }
